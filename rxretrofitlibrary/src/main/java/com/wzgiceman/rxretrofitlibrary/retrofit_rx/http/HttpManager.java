...
public class HttpManager {
    ...
    public HttpManager(HttpOnNextListener onNextListener, RxFragmentActivity fragmentActivity) {
        this.onNextListener = new SoftReference(onNextListener);
        this.fragmentActivity = new SoftReference(fragmentActivity);
    }

    public HttpManager(HttpOnNextSubListener onNextSubListener, RxFragmentActivity fragmentActivity) {
        this.onNextSubListener = new SoftReference(onNextSubListener);
        this.fragmentActivity = new SoftReference(fragmentActivity);
    }
    public HttpManager(HttpOnNextListener onNextListener, RxFragment fragment) {
        this.onNextListener = new SoftReference(onNextListener);
        this.fragment = new SoftReference(fragment);
    }
    public HttpManager(HttpOnNextSubListener onNextSubListener, RxFragment fragment) {
        this.onNextSubListener = new SoftReference(onNextSubListener);
        this.fragment = new SoftReference(fragment);
    }
     ...
     * @param basePar
     */
    public void httpDealForFragmentActivity(Observable observable, BaseApi basePar) {
          /*失败后的retry配置*/
        ...
        observable = observable.retryWhen(new RetryWhenNetworkException(basePar.getRetryCount(),
                ...
                //.compose(appCompatActivity.get().bindToLifecycle())
                //Note:手动设置在activity onDestroy的时候取消订阅
                .compose(fragmentActivity.get().bindUntilEvent(ActivityEvent.DESTROY))
                /*返回数据统一判断*/
                .map(new ResulteFunc())
                /*http请求线程*/
                .subscribeOn(Schedulers.io())
     ...
     * @param basePar
     */
    public void httpDealForFragment(Observable observable, BaseApi basePar) {
          /*失败后的retry配置*/
        ...
        observable = observable.retryWhen(new RetryWhenNetworkException(basePar.getRetryCount(),
                ...
                //.compose(appCompatActivity.get().bindToLifecycle())
                //Note:手动设置在activity onDestroy的时候取消订阅
                .compose(fragment.get().bindUntilEvent(FragmentEvent.DESTROY))
                /*返回数据统一判断*/
                .map(new ResulteFunc())
                /*http请求线程*/
                .subscribeOn(Schedulers.io())
