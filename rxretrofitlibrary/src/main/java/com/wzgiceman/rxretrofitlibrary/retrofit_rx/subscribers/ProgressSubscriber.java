...
public class ProgressSubscriber<T> extends Subscriber<T> {
    ...
    public  ProgressSubscriber(BaseApi api, SoftReference<HttpOnNextListener> listenerSoftReference, SoftReference<Context> mActivity,SoftReference<RxFragment> rxFragment) {
        ...
        if(rxFragment!=null){
            ...
            if (api.isShowProgress()) {
                initProgressDialogForFragment(api.isCancel());
            }
     ...
     * 显示加载框
     */
    private void showProgressDialogForFragment() {
        if (!isShowPorgress()) return;
        Context context = rxFragment.get().getContext();
        if (pd == null || context == null) return;
        if (!pd.isShowing()) {
     ...
     * 初始化加载框
     */
    private void initProgressDialogForFragment(boolean cancel) {
        Context context = rxFragment.get().getContext();
        if (pd == null && context != null) {
            pd = new ProgressDialog(context);
            pd.setCancelable(cancel);
